...
export default function AdultFlipCard() {
    const [cardsA, setCardsA] = useState([]);
    const [turnsA, setTurnsA] = useState(0);
    const [choiceOneA, setChoiceOneA] = useState(null);
    const [choiceTwoA, setChoiceTwoA] = useState(null);
    ...
    const shuffleCards = () => {
        ...
        setChoiceOneA(null);
        setChoiceTwoA(null);
        setCardsA(shuffledCards);
        setTurnsA(0);
    };

    const handleChoice = (card) => {
        choiceOneA ? setChoiceTwoA(card) : setChoiceOneA(card);
    };

    useEffect(() => {
        if (choiceOneA && choiceTwoA) {
            setDisabledA(true);
            if (choiceOneA.word === choiceTwoA.word) {
                // setCards(prevCards => {
    ...
    const resetTurn = (matched) => {
        setChoiceOneA(null);
        setChoiceTwoA(null);
        setTurnsA(prev => prev + (matched ? 0 : 1));
        setDisabledA(false);
    };
    ...
    return (
        <div className="flipcard">
            ...
            <div className='card-grid'>
                {cardsA.map(card => (
                    <AdultCard key={card.id}
                        card={card}
                        handleChoice={handleChoice}
                        flipped={card === choiceOneA || card === choiceTwoA || card.matched}
                        disabled={disabledA}
                        showDefinition={card === choiceTwoA}
                    />
                ))}
                <p style={{ fontSize: '27px', color: 'brown' }}>Misses: {turnsA}</p>
            </div>
            <Random />
        </div>
    );
